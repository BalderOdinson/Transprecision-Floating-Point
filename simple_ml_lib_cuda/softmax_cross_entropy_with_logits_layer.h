...
namespace transprecision_floating_point
{
	namespace simple_ml_lib
	{
			...
			tensor<Type> const& y)
		{
			...
			auto const log_probs = produce(exp_scores, sum_exp_scores, [] DEVICE_FUNCTION(Type first, Type second) { return logf(float(first / second)); });
			...
			tensor<Type> const& y)
		{
			...
			return produce(probs, y, [n] DEVICE_FUNCTION(Type first, Type second) { return (first - second) / n; });
		}
