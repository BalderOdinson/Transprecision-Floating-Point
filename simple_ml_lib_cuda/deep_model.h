...
namespace transprecision_floating_point
{
	namespace simple_ml_lib
	{
		...
		struct deep_model
		{
			...
			double accuracy_from_data(tensor<Type> const& inputs, tensor<Type> const& y_oh);
		...
		double deep_model<Type, Loss>::accuracy(tensor<Type> const& logits,
			tensor<Type> const& y_oh)
		{
			auto y = logits.template argmax<1>().template cast<float>();
			auto y_ = y_oh.template argmax<1>().template cast<float>();
		...
		template <typename Type, typename Loss>
		double deep_model<Type, Loss>::accuracy_from_data(tensor<Type> const& inputs,
			tensor<Type> const& y_oh)
		{
			auto y = this->evaluate(inputs).cast<float>();
			auto y_ = y_oh.template argmax<1>().template cast<float>();
...
#include "../cuda_blas/distributions.h"
		...
		{
			return tensor<Type>::template create_random<transprecision_floating_point::cuda_blas::uniform_distribution>(shape.first, shape.second, transprecision_floating_point::cuda_blas::uniform_distribution(0, 1));
		}
		...
		struct variance_normal_initializer
		{
			...
			tensor<Type> operator()(tensor_shape shape)
			{
				...
				return tensor<Type>::template create_random<transprecision_floating_point::cuda_blas::variance_normal_initializer>(shape.first, shape.second, transprecision_floating_point::cuda_blas::variance_normal_initializer(float(fan_in_)));
			}
